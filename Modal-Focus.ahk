/*
  Modal-Focus
  An AutoHotKey script that focuses modal window for quick control with keyboard shortcuts.


  https://github.com/lightproof/modal-focus
  ...
  Startup parameters:
	...
	-beepall          also beep when the modal window pops up already focused by the OS
	-showtip          display current settings in a tray tooltip at script startup
	-noflash          do not briefly change tray icon when the modal window gets focused
*/
...
; Help window
	...
	( LTrim RTrim0 Join
		...
		`n
		This will display notification each time the modal window gets focused.
		`n`n
		...
		`n
		Same as above, but also display notification if the modal window
		...
		`n
		This will sound two short beeps each time the modal window gets focused.
		`n`n
		...
		-noflash
		`n
		Do not briefly change tray icon when the modal window gets focused.
		)"

		; -beepall
		; `n
		; Same as above, but also beep once when the modal window pops up already focused by the OS.
		; `n`n
...
; Show startup tooltip
	if (A_IsAdmin and startup_tip = 1)
		TrayTip, Modal-Focus %version%, Notify: %notify_lvl_name%`nBeep: %beep%,,4
...
; ================================================================================================
	...
	win_activate(hwnd)
	{
			...
			if (notify_lvl = "1" or notify_lvl = "2")
			{
				...
				TrayTip, Modal-Focus, Window focused,,4
				set_tray_icon(tray_icon)
			}
	...
	win_already_active(hwnd)
	{
		...
		if notify_lvl = 2
			TrayTip, Modal-Focus, Already in focus,,4
	}
...
; Subroutine
	request_process_elevation(args)
	{
		if not A_IsAdmin
		{
			...
			{
				MsgBox, 0x30, Modal-Focus, The program needs to be run as Administrator!
				ExitApp
			}
		}
}
...
; Subroutine
	notify_toggle:
		if A_ThisMenuItem = Beep on focus
		{
			...
			{
				beep = On
				TrayTip, Modal-Focus, Beep: %beep%,,4
				Menu, OptionID, Check, Beep on focus
				SoundBeep, , 100
				SoundBeep, , 100
			}
		...
		if not (A_ThisMenuItem = "Beep on focus")
		{
			...
			Loop, 3
			{
				...
				if A_ThisMenuItem = %notify_lvl_name%
				{
					...
					Menu, OptionID, Check, %notify_lvl_name%
					TrayTip, Modal-Focus, Notify: %notify_lvl_name%,,4
				}
				else
				{
